...
public class RelayBootstrap : MonoBehaviour
{
    ...
    public async UniTask EnsureSignedInAsync()
    {
        if(UnityServices.State != ServicesInitializationState.Initialized)
        {
            ...
            Debug.Log("[RelayBootstrap] Unity Services Initialized");
        }
    ...
    private UnityTransport GetUnityTransport()
    {
        ...
        if (transport == null)
        {
            throw new InvalidOperationException("[RelayBootstrap] UnityTransport 없음");
        }
    ...
    public async UniTask<string> StartHostWithRelayAsync(int maxConnection)
    {
        ...
        Debug.Log($"[RelayBootstrap] Relay Allocation 생성 완료. AllocationId = {allocation.AllocationId}");

        string joinCode = await RelayService.Instance.GetJoinCodeAsync(allocation.AllocationId);
        Debug.Log($"[RelayBootstrap] Join Code 생성 완료. JoinCode = {joinCode}");
        ...
        if (!started)
        {
            throw new Exception("[RelayBootstrap] 호스트 시작 실패");
        }
        Debug.Log("[RelayBootstrap] 호스트 시작 완료");
        return joinCode;
    }
    ...
    public async UniTask StartClientWithRelayAsync(string joinCode)
    {
        ...
        if (!started)
        {
            throw new Exception("[RelayBootstrap] 클라이언트 시작 실패");
        }

        Debug.Log("[RelayBootstrap] 클라이언트 시작 완료");
    }
